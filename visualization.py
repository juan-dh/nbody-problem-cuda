...
def random_sample_particles(data: np.ndarray, max_particles: int | None = None, seed: int = 42):
...
def auto_marker_size(n_particles: int) -> float:
    size = 500.0 / np.sqrt(max(n_particles, 1))
    return max(0.2, min(size, 8.0))


def auto_alpha(n_particles: int) -> float:
    alpha = 3000.0 / max(n_particles, 1)
...
def draw_axes(ax, xlim, ylim, zlim, scale=0.25, lw=1.0, alpha=0.9):
    ...
    # X axis
    ax.plot([-x_len, x_len], [0, 0], [0, 0],
            color="white", linewidth=lw, alpha=alpha)

    # Y axis
    ax.plot([0, 0], [-y_len, y_len], [0, 0],
            color="white", linewidth=lw, alpha=alpha)

    # Z axis
    ax.plot([0, 0], [0, 0], [-z_len, z_len],
            color="white", linewidth=lw, alpha=alpha)


def make_gif(
    input_file: str,
    output_gif: str = "nbody_visualization.gif",
    max_particles: int | None = 5000,
    frame_stride: int = 3,
    fps: int = 15,
    ...
    # Delete previous GIF if it exists
...
if __name__ == "__main__":
    make_gif(
        ...
        output_gif="nbody_visualization.gif",
        max_particles=10000,
        frame_stride=3,
        fps=15,
        marker_size=None,
